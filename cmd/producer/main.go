...
import (
	...
	"github.com/Noviiich/kafka-notify/internal/http-server/handlers/notification/send"
	mwLogger "github.com/Noviiich/kafka-notify/internal/http-server/middleware/logger"
	sendService "github.com/Noviiich/kafka-notify/internal/http-server/services/notification/send"
	"github.com/Noviiich/kafka-notify/internal/kafka/producer"
...
func main() {
	...
	kafkaProducer, err := producer.New(cfg.KafkaConfig)
	if err != nil {
		log.Error("failed to initialize kafka producer %s", sl.Err(err))
		os.Exit(1)
	}
	defer kafkaProducer.Close()
	log.Info("starting producer")

	sendNotificationService := sendService.New(storage, kafkaProducer)
